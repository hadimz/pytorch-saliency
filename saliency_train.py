...
def ev_phase2(_images, _labels):
    ...
    _is_real_label = PT(is_real_label=(_targets == _labels).long())
    _masks, _exists_logits, _ = saliency_p(_images, _targets)
    _masks2, _, _ = saliency_p(torch.multiply(_images, F.upsample(torch.where(_masks>0.5, 1., 0.), (_images.size(2), _images.size(3)), mode='bilinear')), _targets)
    _masks3, _, _ = saliency_p(torch.multiply(_images, 1-F.upsample(_masks, (_images.size(2), _images.size(3)), mode='bilinear')), _targets)
    PT(exists_logits=_exists_logits)
